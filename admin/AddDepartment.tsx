...
const AddDepartment = () => {
  ...
  const handleAddDepartment = async () => {
    if (!code || !name) {
      Alert.alert('Lỗi', 'Mã phòng ban và tên phòng ban không được để trống');
      return;
    }

    try {
      const response = await fetch('http://10.0.2.2:3000/departments', {
        method: 'POST',
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      />
      <Button title="Thêm Phòng Ban" onPress={handleAddDepartment} />
    </ScrollView>
  );
};
