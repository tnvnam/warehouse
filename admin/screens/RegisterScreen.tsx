...
const RegisterScreen: React.FC<Props> = ({ navigation }) => {
  ...
  const fetchOptions = async () => {
    try {
      const depRes = await fetch('http://192.168.1.138:3000/departments');
      const roleRes = await fetch('http://192.168.1.138:3000/roles');
  ...
  const handleRegister = async () => {
    ...
    try {
      setLoading(true);
      const res = await fetch('http://192.168.1.138:3000/users', {
        method: 'POST',
